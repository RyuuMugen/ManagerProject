...
const DemoView = () => {
  ...
  //#region VARIBLE
  const columns: Array<EuiBasicTableColumn<TblTestModel>> = [
    {
      field: "name",
      name: "Tên",
        ...
        <EuiLink
          ...
          onClick={(e: any) => {
            ...
            if (isNullOrUndefined(username))
              ...
              const id = users.find((x) => x.name === username);
      ...
      mobileOptions: {
        render: (user: TblTestModel) => <span>{user.name}</span>,
        header: false,
        truncateText: false,
        enlarge: true,
        width: "100%",
    ...
    },
    {
      ...
      name: "Trạng thái",
      dataType: "boolean",
      render: (online: TblTestModel["inactive"]) => {
  ...
  const [users, setUsers] = useState<TblTestModel[]>([]);
  ...
  const [isSelectedItem, setSelectedItems] = useState<TblTestModel[]>([]);
  ...
  const onTableChange = async ({
    page: { index, size },
  }: CriteriaWithPagination<TblTestModel>) => {
  ...
  const onSelectionChange = (selectedItems: TblTestModel[]) => {
    setSelectedItems(selectedItems);
  };
  const selection: EuiTableSelectionType<TblTestModel> = {
    // tieu chi de checkbox
    selectable: (user: TblTestModel) => true,
  ...
  const openModal = () =>
    modals.openConfirmModal({
      title: (
        <>
          <div color="white">
            <Title order={5}>Thêm mới demo !</Title>
          </div>
        </>
      ),
      children: (
